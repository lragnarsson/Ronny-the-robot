...
void i2c_init(uint8_t bitrate, uint8_t prescaler, uint8_t adress)
{
	...
	TWCR = (1<<TWEN) | (1<<TWEA) | (1<<TWIE);	//enable TWI, TWI interrupts, TWI enable ack
	sei();
}
...
ISR(TWI_vect) {
	switch (TWSR & 0xf8) {
		...
		cli();
		busbuffer = TWDR;
		TWCR = (0<<TWSTA)|(1<<TWSTO)|(1<<TWINT); //Send ack
		while (!(TWCR & (1<<TWINT)));
		sei();
		break;
		...
		case 0x98: // Previously addressed with general call, data has been received, NOT ACK has been returned
			cli();
			TWCR = (1<<TWSTA)|(0<<TWSTO)|(1<<TWINT)|(1<<TWEA); // Switched to not adressed slave mode
			while (!(TWCR & (1<<TWINT)));
			sei();
			break;
		case 0xA0: // A STOP condition or repeated START condition has been received while still addressed as Slave
			cli();
			TWCR = (0<<TWSTA)|(0<<TWSTO)|(1<<TWINT)|(1<<TWEA); // Switched to the not addressed Slave mode, own SLA will be recognized
			while (!(TWCR & (1<<TWINT)));
			sei();
			break;
		// Misc. states
